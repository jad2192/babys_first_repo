# -*- coding: utf-8 -*-
"""
Created on Tue Sep 25 14:36:08 2018
...
def discretize2(counts, cnt='day'):
    mu = np.median(counts)
    sigs = []
    if cnt == 'day':
        L = 7
...
class EARS(object):
    ...
    def predict_anomaly(self, ws=7):
        
        if self.c == 3:
            
            c3 = (max(0, self.get_c_stat(-1,ws)-1) +
                  max(0, self.get_c_stat(-2,ws)-1) +
                  max(0, self.get_c_stat(-3,ws)-1))
