...
###############################################################################
''' The following function creates a dictionary of brands to query from twitter
    from a csv file containing the brands along with their twitter handles
    and associated hashtags. The resulting dictionary has the brands as keys
    and a string tying together all the hashtags and handles into a query for
    the twitter api. E.g  brands['apple'] = '@apple OR @applecare OR #iPhone...'
'''
def get_brands(fp):
...
def get_all_tweets(brands, since_date, until_date):
    ...
    for brand in brand_set:
        ...
        for x in tweets:
            ...
            # twitter_dnn is a pretrained deep learning model to filter out relevant tweets.
            # Phase 2 model is another pretrained ML model used for further filtering.
...
def safety_flag_analysis(date_out):
    '''This function runs a time series analysis on daily tweet frequencies
       and raises a flag when there is an anomalous increase. 
    '''
